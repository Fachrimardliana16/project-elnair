...
class UpdateLandingPageRequest extends FormRequest
{
    ...
    {
        $pageId = $this->route('landing_page')?->id ?? $this->route('landing_page');

        return [
            'title'                   => 'required|string|max:255',
            'slug'                    => 'nullable|string|max:255|unique:landing_pages,slug,' . $pageId . '|regex:/^[a-z0-9\-]+$/',
            'custom_domain'           => 'nullable|string|max:255|unique:landing_pages,custom_domain,' . $pageId,
