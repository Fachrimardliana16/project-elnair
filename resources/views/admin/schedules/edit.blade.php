...
@section('title', 'Ubah Jadwal Keberangkatan')
@section('page_title', 'Ubah Jadwal Keberangkatan')
    ...
    <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="package_id" id="package_id" class="form-control @error('package_id') is-invalid @enderror" required style="padding: 0.75rem 1rem;">
                @foreach($packages as $package)
                    <option value="{{ $package->id }}" {{ old('package_id', $schedule->package_id) == $package->id ? 'selected' : '' }}>
        ...
        <div class="grid-2">
            <div class="form-group">
                ...
                <input type="date" name="departure_date" id="departure_date" class="form-control @error('departure_date') is-invalid @enderror" value="{{ old('departure_date', $schedule->departure_date->format('Y-m-d')) }}" required style="padding: 0.75rem 1rem;">
            ...
            <div class="form-group">
                ...
                <input type="number" name="available_seats" id="available_seats" class="form-control @error('available_seats') is-invalid @enderror" value="{{ old('available_seats', $schedule->available_seats) }}" min="0" required style="padding: 0.75rem 1rem;">
        ...
        <div class="grid-2">
            <div class="form-group">
                ...
                <select name="status" id="status" class="form-control @error('status') is-invalid @enderror" required style="padding: 0.75rem 1rem;">
                    ...
                    <option value="Hampir Penuh" {{ old('status', $schedule->status) === 'Hampir Penuh' ? 'selected' : '' }}>Hampir Penuh</option>
                    <option value="Penuh" {{ old('status', $schedule->status) === 'Penuh' ? 'selected' : '' }}>Penuh / Sold Out</option>
                </select>
                @error('status')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group" style="display: flex; align-items: center; margin-top: 2rem;">
                <label style="display: flex; align-items: center; gap: 8px; font-weight: 600; cursor: pointer; user-select: none;">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $schedule->is_active) ? 'checked' : '' }} style="width: 18px; height: 18px; accent-color: var(--brand-dark);">
